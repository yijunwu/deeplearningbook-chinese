...
对于许多图像数据集，还有一些分别的基准，一些是针对那些具有\firstgls{permutation_invariant}并且必须通过学习发现拓扑结构的模型，另一些是针对模型设计者将空间关系的知识植入了它们的模型。
...
组合方法显著快于使用它们的外积来执行一个朴素的$d$维的卷积。
并且核也只要更少的参数来表示成向量。
...
如多层感知机一样，我们使用\gls{greedy_layer_wise_pretraining}，单独训练第一层，然后一次性地从第一层提取所有特征，之后用那些特征单独训练第二层，以此类推。
...
卷积模型的\gls{greedy_layer_wise_pretraining}的经典例子是卷积\gls{DBN}\citep{HonglakL2009}。
...
这可能可以简单地归结为\gls{convolutional_network}比全连接网络计算效率更高，因此更容易运行多个实验并调整它们的实现和超参数。
更大的网络也似乎更容易训练。
利用现代硬件，大型全连接的网络在许多任务上也表现得很合理，即使使用过去那些全连接网络被认为不能在其上工作得很好的数据集和当时流行的激活函数，现在也能执行得很好。
