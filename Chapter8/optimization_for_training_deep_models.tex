...
不难将这种\gls{supervised_learning}扩展成其他形式，如函数$L$包括$\Vtheta$或者$\Vx$作为参数，或是去掉参数$y$，以发展不同形式的\gls{regularization}或是\gls{unsupervised_learning}。
% 268 mid
...
这是因为即使$0-1$损失期望是零时，我们还能拉开不同类别的距离以改进分类器的鲁棒性，获得一个更强壮的、更值得信赖的分类器，从而，相对于简单地最小化\gls{training_set}上的平均$0-1$损失，它能够从训练数据中提取更多信息。
% 270 head
...
% -- 276 mid
尽管\gls{ill_conditioning}也存在于除了\gls{NN}训练的其他情况中，有些适用于其他情况的解决\gls{ill_conditioning}的技术并不适用于\gls{NN}。
...
我们可以构建甚至没有\gls{hidden_unit}的小规模\gls{NN}，其\gls{local_minima}的\gls{cost}比\gls{global_minimum}的\gls{cost}大很多~\citep{Sontag-cs89,Brady89,Gori-pami91}。
如果具有很大\gls{cost}的\gls{local_minima}是常见的，那么这将给基于梯度的优化算法带来严重的问题。
% -- 277 end
...
\cite{GoodfellowOptimization15}可视化了最新(State Of The Art)\gls{NN}的几个学习轨迹，\figref{fig:chap8_plot_atmu_relu_5}\,给了一个例子。
...
迄今为止，我们讨论的许多问题都是关于\gls{loss_function}在单个点的性质——若$J(\Vtheta)$在当前点$\Vtheta$具有\gls{poor_conditioning}，或者$\Vtheta$在悬崖中，或者$\Vtheta$是一个下降方向不明显的\gls{saddle_points}，那么会很难更新当前步。
...
求解\gls{differential_equation}的一个简单数值方法是欧拉方法，通过在每个梯度方向上小且有限的一步来（调整速度进而）简单模拟该等式定义的动力学。
% 290 mid
...
反过来，如果我们初始化$\Vtheta_0$为很大的值，那么我们的先验指定了哪些单元应互相交互，以及它们应如何交互。
% 294 end
...
其结果是，\textbf{\gls{nonlinear_CG}法}\,算法会包括一些偶尔的重设，即\gls{CG}法沿未修改的梯度重启线搜索。
...
针对神经网络的\gls{CG}法应用早已被提出，例如缩放的\gls{CG}法\citep{Moller}。
...
拟\gls{newton_method}所采用的方法（\gls{BFGS}\,是其中最突出的）是使用矩阵$\MM_t$近似逆，迭代地做低秩更新以更好地近似$\MH^{-1}$。
...
使用贪心解初始化联合优化算法，可以极大地加速算法，并提高找到的解的质量。
...
一般情况下，\gls{pretraining}对于优化和\gls{generalization}两者都是有帮助的。
