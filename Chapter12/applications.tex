...
\citet{bengio2013estimating}提出使用\gls{gater}概率梯度的若干估计器，而~\citet{Bacon-et-al-RLDM2015,BengioE-et-al-arXiv2015}使用\gls{RL}技术（\firstgls{policy_gradient}）来学习一种（作用于\gls{hidden_unit}块）~\gls{dropout}~的条件形式，减少了实际的计算成本，而不会对近似的质量产生负面影响。
% 438
...
使用动态结构系统的主要障碍是由于系统针对不同输入的不同代码分支导致的并行度降低。
...
这样的情况对于学习算法来说可能是困难的。
此外，许多浅层的图模型把多个分离的模式表示在一条线上会有问题。
...
\gls{bag_of_words}具有稀疏向量$\Vv$，其中$v_i$表示词汇表中的词$i$有没有出现在文档中。
...
一些其他模型，例如平铺\gls{convolutional_network}，可以在增加参数的同时增加\footnote{译者：原文为减少，但综合判断疑应为增加}\gls{parameter_sharing}程度，以保持相同的计算量。然而，基于矩阵乘法的典型\gls{NN}层需要与参数数量成比例的计算量。
