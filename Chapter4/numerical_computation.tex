...
\gls{lipschitz}~连续函数的变化速度以一个\firstgls{lipschitz_constant} $\CalL$为界：
\begin{align}
...
为了获得关于这个想法的一些直观解释，我们可以说要么这个解是由不等式强加的边界，我们必须通过对应的~\glssymbol{KKT}~乘子影响$\Vx$的解，要么不等式对解没有影响，我们则归零~\glssymbol{KKT}~乘子。
...
求解线性方程和调整$\lambda$的过程将一直持续到$\Vx$具有正确的范数，同时关于$\lambda$的\gls{derivative}是$0$。
