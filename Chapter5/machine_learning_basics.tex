...
% -- 98 --
    ...
    参考~\cite{chandola2009anomaly}了解异常检测方法。
...
在多实例学习中，\gls{example:chap5}的整个集合被标记为含有或者不含有某类的\gls{example:chap5}，但是集合中单独的样本是没有标记的。
...
由于验证集是用来``训练''超参数的，验证集误差通常会低估泛化误差，尽管低估的程度比训练集要小些。
所有超参数优化完成之后，泛化误差可以通过\gls{test_set}来估计。

在实际中，当相同的\gls{test_set}已在很多年中重复地用于评估不同算法的性能，并且考虑学术界在该\gls{test_set}上的各种尝试，我们最后可能在\gls{test_set}上也会有偏乐观的估计。
\gls{benchmarks}会因之变得过时，而不能反映系统的真实性能。
值得庆幸的是，学术界往往会移到新的（通常会更巨大、更具挑战性）基准\gls{dataset}上。

\subsection{交叉验证}
\label{sec:cross_validation}
将\gls{dataset}分成固定的训练集和固定的\gls{test_set}后，若\gls{test_set}的样本数量很少，这将是有问题的。
